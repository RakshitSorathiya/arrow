...
// HCons(head=1000, tail=HCons(head=900, tail=arrow.generic.HNil@f7fd680))
```

```kotlin
Account(1000, 900).toHListLabeled()
// HCons(head=Tuple2(a=balance, b=1000), tail=HCons(head=Tuple2(a=available, b=900), tail=arrow.generic.HNil@f7fd680))
```
...
// DeferredK(deferred=LazyDeferredCoroutine{New}@2ba2d4b5)
```
